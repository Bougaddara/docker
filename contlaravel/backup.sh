...
config_phpmyadmin(){
    ...
    sed -i "s/\$dbserver='localhost'/\$dbserver='5.189.148.45'/g" /etc/phpmyadmin/config-db.php
    sed -i "s/localhost/5.189.148.45/g" /usr/share/phpmyadmin/libraries/config.default.php
}
...
config_laravel(){
    ...
    #sed -i 's/DB_PASSWORD=/DB_PASSWORD='${PASSWD}'/g' .env
